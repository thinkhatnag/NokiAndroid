...
class VideoReporter extends WdioReporter {
    ...
    onBeforeCommand() {
        ...
        __classPrivateFieldGet(this, _VideoReporter_instances, "m", _VideoReporter_attachVideoToAllure).call(this);
    }
    ...
    onTestPass(payload) {
        if (!__classPrivateFieldGet(this, _VideoReporter_record, "f") || !this.isCucumberFramework) {
            return;
        }
        this.onTestEnd(payload);
    }
    ...
    addFrame() {
        ...
        if (browser.isBidi) {
            browser.getWindowHandle().then((contextId) => {
                const contextCaptureScreenshotParameters = {
                    context: contextId,
                    origin: 'viewport',
                    format: { type: 'image/png' }
                };
                this.screenshotPromises.push(browser.browsingContextCaptureScreenshot(contextCaptureScreenshotParameters)
                    .then((captureScreenshotResult) => {
                    __classPrivateFieldGet(this, _VideoReporter_instances, "m", _VideoReporter_log).call(this, `- Bidi Screenshot (frame: ${frame})`);
                    fs.writeFileSync(filePath, captureScreenshotResult.data, 'base64');
                }).catch((error) => {
                    fs.writeFileSync(filePath, notAvailableImage, 'base64');
                    __classPrivateFieldGet(this, _VideoReporter_instances, "m", _VideoReporter_log).call(this, `Bidi Screenshot not available (frame: ${frame}). Error: ${error}..`);
                }));
            });
        }
        else {
...
}, _VideoReporter_attachVideoToAllure = function _VideoReporter_attachVideoToAllure() {
    if (!__classPrivateFieldGet(this, _VideoReporter_usingAllure, "f") || !this.testName || !__classPrivateFieldGet(this, _VideoReporter_record, "f")) {
        return;
    }
    const formatSettings = getVideoFormatSettings(this.options.videoFormat);
